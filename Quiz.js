...
const questionBank = {
  "Web Development": [{
    ...
    questionText: "What is the main purpose of an SQL database?",
    choices: ["To handle front-end logic", "To store and manage relational data", "To create user interfaces", "To style web pages"],
    correctAnswer: "To store and manage relational data"
}, {
    ...
    questionText: "Which of the following is used to store data in a web browser?",
    choices: ["Cookies", "Sessions", "Local Storage", "All of the Above"],
  ...
  History: [
    ...
    {
      questionText:
        "Which war was fought between the North and South in the US?",
...
function setupEventListeners() {
  ...
  elements.categorySelect.addEventListener("change", () => {
    ...
    elements.startButton.classList.toggle(
      "enabled",
...
function updateTimerUI() {
  ...
  elements.timerBar.style.width = `${
    (quizState.timeLeft / quizState.TIME_LIMIT) * 100
  }%`;
}

function handleAnswer(selectedChoice, clickedButton) {
  clearInterval(quizState.timerInterval);
  Array.from(elements.optionsList.children).forEach(
    (btn) => (btn.disabled = true)
  );

  const currentQuestion = quizState.questions[quizState.currentQuestionIndex];
  let isCorrect = (selectedChoice === currentQuestion.correctAnswer);
  ...
  if (isCorrect) {
    ...
    const correctButton = Array.from(elements.optionsList.children).find(
  ...
  quizState.userAnswers.push({
    ...
    userAnswer: selectedChoice || "No answer (Time out)",
...
function finishQuiz() {
  ...
  let resultsHTML = `
        <h2 style="font-size: 1.5rem; color: #111827; margin-bottom: 20px;">Your Final Score: ${quizState.score} / ${quizState.questions.length}</h2>
        <div class="results-summary" style="text-align: left;"> <h2 style="text-align: center">Overall Result Summary</h2>
    `;

  quizState.userAnswers.forEach((result, index) => {
    const statusIcon = result.isCorrect ? "&#9989;" : "&#10060;";
    const userAnswerClass = result.isCorrect ? "correct-text" : "wrong-text";
    resultsHTML += `
            <div style="margin-bottom: 25px; padding: 15px; border-radius: 8px; background: #f1f5f9;">
                <p><strong>Question ${index + 1}:</strong> ${
      result.question
    }</p>
                <p><strong>Your Answer:</strong> <span class="${userAnswerClass}">${
      result.userAnswer
    } ${statusIcon}</span></p>
